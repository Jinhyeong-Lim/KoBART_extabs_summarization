...
def eval(model, tokenizer,test_loader, device):
    ...
    with torch.no_grad():
        ...
        for text, ans in test_loader:
            ...
            test_doc = [tokenizer.encode_plus(t, add_special_tokens=True, max_length = 1024, pad_to_max_length = True)["input_ids"] for t in text]
            ...
            summary_ids = model.generate(test_doc,
                                         ...
                                         length_penalty= 1.0, min_length=1, max_length=100
                                         ).to(device)
    ...
    return total_r1 / len(test_loader), total_r2 / len(test_loader), total_rl / len(test_loader)
